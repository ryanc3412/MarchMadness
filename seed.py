...
def simulate_round(current, next_round, round_name, index=None):
    """Simulates a single round using a probability model based on seed rankings."""
    print(f"\n{round_name}")
    winners = []
    for i in range(0, len(current), 2):
        seed1, seed2 = current[i], current[i + 1]
        winner = weighted_winner(seed1, seed2)
        winners.append(winner)
        print(f"{seed1} vs {seed2} -> Winner: {winner}")
...
def weighted_winner(seed1, seed2):
    """Determines the winner based on seed-weighted probability."""
    probability_seed1_wins = seed2 / (seed1 + seed2)
    return seed1 if random.random() < probability_seed1_wins else seed2